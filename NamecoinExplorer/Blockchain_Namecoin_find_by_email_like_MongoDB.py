...
try:
    from ontology import (
        ...
        Attributes, IP, Domain, IPToDomain, Email, EmailToDomain)
...
def return_massive_about_emails_like(emails, limit, server, user, password):
...
class NamecoinDomainExplorerEmail(metaclass=Header):
    display_name = 'Namecoin Explorer(with emails)'
...
class NamecoinDomainIP(metaclass=Schema):
    name = 'Namecoin schema: Domain, IP, Email'
    Header = NamecoinDomainExplorerEmail

    SchemaIP = SchemaObject(IP, mapping={IP.IP: Header.ip})
    SchemaDomain = SchemaObject(Domain, mapping={Domain.Domain: Header.domain})

    SchemaEmail = SchemaObject(Email, mapping={Email.Email: Header.email})
    ...
    SchemaEmailToDomain = EmailToDomain.between(SchemaEmail, SchemaDomain,
                                                mapping={EmailToDomain.DateTime:Header.date_time},
                                                conditions=[not_empty(Header.domain), not_empty(Header.email)])



class NamecoinHistoryDomainIPLikeEmailMongoDB(Task):
    ...
    def get_id(self):
        return '92d84c54-ac26-4540-ac6c-d2207e1435b3'
    ...
    def get_headers(self):
        return HeaderCollection(NamecoinDomainExplorerEmail)
    ...
    def get_enter_params(self):
        ...
        ep_coll.add_enter_param('emails', 'Emails', ValueType.String, is_array=True, required=True,
                                value_sources=[Attributes.System.Email],
                                description='email, e.g.:\nprotonmail.com')
    ...
    def execute(self, enter_params, result_writer, log_writer, temp_dir=None):
        ...
        for line in sorted(result_lines,  key=lambda line: line['date_time']):
            ...
            result_writer.write_line(tmp, header_class=NamecoinDomainExplorerEmail)
            i += 1
...
if __name__ == '__main__':
    ...
    NamecoinHistoryDomainIPLikeEmailMongoDB().execute(EnterParamsFake, WriterFake, WriterFake, None)